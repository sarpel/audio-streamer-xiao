...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.restart_required) {
        showAlert(
          "Device restart required. Please restart manually.",
          "warning"
        );
      }
    } catch (error) {
      showAlert(
        "Failed to save buffer configuration: " + error.message,
        "error"
      );
    }
  });
...
document.getElementById("debug-form")?.addEventListener("submit", async (e) => {
  ...
  try {
    const data = {
      ...
      stack_monitoring: document.getElementById("debug-stack-monitoring")
        .checked,
